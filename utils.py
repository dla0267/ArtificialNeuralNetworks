...
class train_data():
    ...
    def __enter__(self):
        ...
        #self.data= mmap.mmap(f.fileno(), 0, prot=mmap.PROT_READ)
        self.data= np.memmap(self.filepath, dtype= 'f', mode= 'r', shape= (156850,64,64,1) )
        #self.data = np.load(file= self.filepath)
...
def cal_psnr(clean, noisy):
    mse = ((noisy- clean) ** 2).mean()
    psnr = 10*np.log10(np.amax(clean)**2/mse)
    return psnr
